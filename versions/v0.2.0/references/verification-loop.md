...
## v0.2 diagnostic outputs

The comparison script also creates:

- `diff-mask.png` — thresholded mismatch mask;
- `annotated-mismatch.png` — actual image with the overall mismatch bounds;
- mismatch bounding coordinates;
- highest-mismatch rows and columns;
- ranked regional grid metrics.

Use regional diagnostics to decide where to inspect first. A low global error can still hide a severe mismatch in one important component.
